...
class RoadRStandardRoIHead(BaseRoIHead):
    ...
    def _bbox_forward(self, x: Tuple[Tensor], rois: Tensor) -> dict:
        ...
        cls_score, act_score, loc_score, bbox_pred = self.bbox_head(bbox_feats)

        bbox_results = dict(
            cls_score=cls_score, act_score=act_score, loc_score=loc_score, bbox_pred=bbox_pred, bbox_feats=bbox_feats)
    ...
    def bbox_loss(self, x: Tuple[Tensor],
        ...
        bbox_loss_and_target = self.bbox_head.loss_and_target(
            ...
            act_score=bbox_results['act_score'],
            loc_score=bbox_results['loc_score'],
    ...
    def predict_bbox(self,
        ...
        act_scores = bbox_results['act_score']
        loc_scores = bbox_results['loc_score']
        bbox_preds = bbox_results['bbox_pred']
        num_proposals_per_img = tuple(len(p) for p in proposals)
        rois = rois.split(num_proposals_per_img, 0)
        cls_scores = cls_scores.split(num_proposals_per_img, 0)
        act_scores = act_scores.split(num_proposals_per_img, 0)
        loc_scores = loc_scores.split(num_proposals_per_img, 0)
