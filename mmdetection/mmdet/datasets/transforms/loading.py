...
@TRANSFORMS.register_module()
class LoadMultiClassAnnotations(LoadAnnotations):
    """Load and process the ``instances`` and ``seg_map`` annotation provided
    by dataset. It must load ``instances_ids`` which is only used in the
    tracking tasks. The annotation format is as the following:
    ...
    .. code-block:: python
        {
            ...
            'gt_bboxes': np.ndarray(N, 4)
             # In int type.
            'gt_bboxes_labels': np.ndarray(N, )
            'gt_agent_labels': np.ndarray(N, 1)
            'gt_action_labels': np.ndarray(N, 1)
            'gt_location_labels': np.ndarray(N, 1)
             # In built-in class
            'gt_masks': PolygonMasks (H, W) or BitmapMasks (H, W)
             # In uint8 type.
            'gt_seg_map': np.ndarray (H, W)
    ...
    - gt_bboxes (np.float32)
    - gt_bboxes_labels (np.int32)
    - gt_agent_labels (np.float32)
    - gt_action_labels (np.float32)
    - gt_location_labels (np.float32)
    """
    ...
    def __init__(
            ...
            ignore_index: int = 255,
            **kwargs) -> None:
        super(LoadMultiClassAnnotations, self).__init__(**kwargs)
        self.with_mask = with_mask
        self.poly2mask = poly2mask
        self.box_type = box_type
        self.multi_label = multi_label
    ...
    def _load_multi_label(self, results: dict) -> None:
        """Private function to load label annotations.
...
#         print(results.keys())
        for instance in results.get('instances', []):
            gt_agent_labels.append(instance['agent_label'])
            gt_action_labels.append(instance['action_label'])
            gt_location_labels.append(instance['location_label'])
        # TODO: Inconsistent with mmcv, consider how to deal with it later.
        results['gt_agent_labels'] = np.array(
            gt_agent_labels, dtype=np.int64)
        results['gt_action_labels'] = (np.array(
            gt_action_labels, dtype=np.int64))
        results['gt_location_labels'] = (np.array(
            gt_location_labels, dtype=np.int64))
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        results = super().transform(results)
        self._load_multi_label(results)
        return results
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'(with_bbox={self.with_bbox}, '
        repr_str += f'with_label={self.with_label}, '
        repr_str += f'multi_label={self.multi_label}, '
        repr_str += f'with_mask={self.with_mask}, '
        repr_str += f'with_seg={self.with_seg}, '
        repr_str += f'poly2mask={self.poly2mask}, '
        repr_str += f"imdecode_backend='{self.imdecode_backend}', "
